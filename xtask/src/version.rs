...
pub fn get_git_version_info() -> Result<GitVersionInfo, Box<dyn std::error::Error>> {
    ...
    if let Ok(tags) = inner_repo.tag_names(None) {
        for tag_name in tags.iter().flatten() {
            if let Ok(peeled) = inner_repo
                .revparse_single(tag_name)
                .and_then(|obj| obj.peel_to_commit())
            {
                let is_ancestor = inner_repo
                    .graph_descendant_of(head_oid, peeled.id())
                    .unwrap_or(false);
    ...
    let commits_opt = if !range.is_empty() {
        repo.commits(Some(&range), None, None, false).ok()
    } else {
        None
    };

    if let Some(commits) = commits_opt {
        for commit in commits {
            ...
            if let Ok(conv_commit) = cliff_commit.into_conventional() {
                ...
                let type_part = conv_commit
                    ...
                    .unwrap_or_else(|| {
                        let type_str = first_line.split(':').next().unwrap_or("").trim();
                ...
                if !is_breaking {
                    ...
                    } else if (type_part == "fix" || type_part == "refactor" || type_part == "perf")
                        && bump == "none"
                    {
                        bump = "patch";
                    }
